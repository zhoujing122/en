...
#include "robot_slamd/mapping/spin_scan_localization.hpp"
#include "robot_slamd/sensors/tof.hpp"
#include "robot_slamd/sensors/tof_pose_correction.hpp"
...
int real_main(int argc, char **argv) {
    ...
    auto update_yaw_correction_post_apply = [&](double now_s, bool apply_happened, const YawCorrectionApplySnapshot &apply_snap,
                                                const YawCorrectionGateSnapshot &gate_snap, const SparseScanYawMatchSummary &ys) {
        if (!cfg.yaw_correction_post_apply_enabled) return;
        YawCorrectionPostApplyInput pin;
        pin.timestamp_s = now_s;
        pin.apply_happened = apply_happened;
        pin.applied_scan_id = apply_snap.gate_match_scan_id;
        pin.applied_match_timestamp_s = apply_snap.gate_match_timestamp_s;
        pin.applied_delta_deg = apply_snap.delta_yaw_deg;
        pin.latest_yaw_match = ys;
        pin.latest_gate = gate_snap;
        pin.supervisor = latest_supervisor_snapshot;
        bool changed = yaw_correction_post_apply.update(pin);
        if (yaw_correction_post_apply_log && (changed || apply_happened)) write_yaw_correction_post_apply_row(yaw_correction_post_apply_log, yaw_correction_post_apply.snapshot());
    };

    auto update_yaw_correction_apply_from_gate = [&](double now_s, const YawCorrectionGateSnapshot &gate_snap, const SparseScanYawMatchSummary &ys) {
        if (!cfg.yaw_correction_enabled || cfg.yaw_correction_mode != "writeback" || !cfg.yaw_correction_writeback_enabled) {
            YawCorrectionApplySnapshot no_apply;
            update_yaw_correction_post_apply(now_s, false, no_apply, gate_snap, ys);
            return;
        }
        const auto &p = loc.pose();
        ...
        if (apply_snap.applied) {
            ...
            if (yaw_correction_gate_log) {
                YawCorrectionGateSnapshot feedback_snap = yaw_correction_gate.snapshot();
                write_yaw_correction_gate_row(yaw_correction_gate_log, feedback_snap);
                yaw_correction_gate.mark_logged(now_s, feedback_snap);
            }
        }
        update_yaw_correction_post_apply(now_s, apply_snap.applied, apply_snap, gate_snap, ys);
    };
    ...
    auto update_yaw_correction_gate_from_summary = [&](double now_s, const SparseScanYawMatchSummary &ys, bool force_log) {
        ...
        update_yaw_correction_apply_from_gate(now_s, snap, ys);
    };
    ...
    auto update_recovery_manager_from_snapshots = [&](double now_s, bool force_log) {
        if (!cfg.recovery_enabled || cfg.recovery_mode == "disabled") return;
        RecoveryManagerInput rin;
        rin.timestamp_s = now_s;
        rin.localizer_initialized = loc.initialized();
        rin.current_pose = loc.pose();
        rin.linear_speed_mps = loc.v();
        rin.yaw_rate_radps = loc.w();
        rin.map_quality = latest_map_quality_snapshot;
        rin.supervisor = latest_supervisor_snapshot;
        rin.active_scan = latest_active_scan_snapshot;
        rin.active_scan_command = latest_active_scan_command_snapshot;
        rin.latest_yaw_match = latest_yaw_match_summary;
        rin.latest_yaw_gate = yaw_correction_gate.snapshot();
        rin.latest_yaw_apply = yaw_correction_apply.latest_snapshot();
        rin.latest_post_apply = yaw_correction_post_apply.snapshot();
        rin.latest_tof_age_s = latest_tof_sample_s >= 0.0 ? now_s - latest_tof_sample_s : std::numeric_limits<double>::infinity();
        rin.tof_recent = latest_tof_sample_s >= 0.0 && rin.latest_tof_age_s <= cfg.recovery_max_tof_age_s;
        rin.map_known_cells = latest_map_quality_snapshot.known_cells;
        rin.map_occupied_cells = latest_map_quality_snapshot.occupied_cells;
        bool changed = recovery_manager.update(rin);
        latest_recovery_snapshot = recovery_manager.snapshot();
        if (recovery_manager_log && (force_log || changed || recovery_manager.should_log(now_s))) {
            write_recovery_manager_row(recovery_manager_log, latest_recovery_snapshot);
            recovery_manager.mark_logged(now_s, latest_recovery_snapshot);
        }
    };
    ...
    if (cfg.map_quality_enabled || cfg.mapping_supervisor_enabled || cfg.active_scan_enabled || cfg.active_scan_execution_enabled || cfg.sparse_scan_enabled || cfg.sparse_scan_yaw_match_enabled || cfg.yaw_correction_enabled || cfg.recovery_enabled || cfg.motion_execution_enabled) {
        ...
        if (cfg.yaw_correction_enabled) {
            ...
            metrics.yaw_correction_post_apply = yaw_correction_post_apply.run_stats();
        }

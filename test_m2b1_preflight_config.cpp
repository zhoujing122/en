...
int main() {
    ...
    const std::string cfg_path = source_dir() + "/config.sim.yaml";
    ...
    write_resolved_config(loopback, resolved);
    std::ifstream in(resolved);
    std::ostringstream ss;
    ss << in.rdbuf();
    std::string text = ss.str();
    expect(text.find("m2b1_preflight:") != std::string::npos, "resolved preflight block");
    expect(text.find("production_transport_backend: loopback_shadow") != std::string::npos, "resolved transport backend");

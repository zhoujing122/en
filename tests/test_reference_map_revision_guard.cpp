...
int main() {
    ...
    SparseMapRevisionGuard guard;
    expect(guard.unchanged(9, 9), "inactive guard passes");
    guard.active = true;
    guard.reference_map_revision = 4;
    guard.reference_map_cell_count = 12;
    expect(guard.unchanged(4, 12), "same revision passes");
    expect(!guard.unchanged(5, 12), "revision change fails");
    expect(!guard.unchanged(4, 13), "cell count change fails");
    auto c = controller();
    expect(c.begin_collection(4, 12, identity_map_from_odom()).ok, "begin guard");
    expect(c.verify_reference_map(4, 12).ok, "controller guard passes");
    auto changed = c.verify_reference_map(4, 13);
    expect(!changed.ok && changed.fault == ActiveObservationBundleFault::ReferenceMapChangedDuringActiveBundle, "controller guard aborts");
    expect(c.state() == ActiveObservationBundleState::Aborted, "controller aborted");
    return 0;
}

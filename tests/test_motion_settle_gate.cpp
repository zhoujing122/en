...
int main() {
    ...
    cfg.min_stable_duration_s = 0.2;
    MotionSettleGate gate(cfg);
    auto a = gate.update(settle_sample(1.0));
    expect(a.ok && !a.stable && a.consecutive_sample_count == 1, "first stable sample");
    auto b = gate.update(settle_sample(1.1));
    expect(b.ok && !b.stable && b.consecutive_sample_count == 2, "second sample not enough duration");
    auto c = gate.update(settle_sample(1.21));
    expect(c.ok && c.stable && c.consecutive_sample_count == 3, "samples and duration stable");
    Config config;
    validate_config(config);
    config.sparse_slam_settle_min_consecutive_samples = 0;
    bool threw = false;
    try { validate_config(config); } catch (...) { threw = true; }
    expect(threw, "invalid settle config rejected");
    return 0;
}

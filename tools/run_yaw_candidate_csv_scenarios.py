#!/usr/bin/env python3
import argparse
import csv
import math
import shutil
import subprocess
from pathlib import Path

TOF_EXTRINSICS = {
    "front": (0.10, 0.00, 0.0),
    "left": (0.00, 0.08, math.radians(90.0)),
    "right": (0.00, -0.08, math.radians(-90.0)),
}


def write_rows(path, header, rows):
    with Path(path).open("w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(header)
        writer.writerows(rows)


def wrap_pi(a):
    while a > math.pi:
        a -= 2.0 * math.pi
    while a < -math.pi:
        a += 2.0 * math.pi
    return a


def ray_segment(px, py, yaw, x1, y1, x2, y2):
    dx, dy = math.cos(yaw), math.sin(yaw)
    sx, sy = x2 - x1, y2 - y1
    det = -dx * sy + dy * sx
    if abs(det) < 1e-9:
        return None
    bx, by = x1 - px, y1 - py
    t = (-sy * bx + sx * by) / det
    u = (-dy * bx + dx * by) / det
    if t > 0.0 and 0.0 <= u <= 1.0:
        return t
    return None


def ray_world(px, py, yaw, segments, max_range=3.0):
    best = 1e9
    for seg in segments:
        t = ray_segment(px, py, yaw, *seg)
        if t is not None and t < best:
            best = t
    return best if best < 1e8 else max_range + 1.0


def tof_ranges(pose, segments, max_range=3.0):
    x, y, yaw = pose
    out = {}
    cy, sy = math.cos(yaw), math.sin(yaw)
    for sensor_id, (ex, ey, eyaw) in TOF_EXTRINSICS.items():
        sx = x + cy * ex - sy * ey
        syw = y + sy * ex + cy * ey
        r = ray_world(sx, syw, yaw + eyaw, segments, max_range)
        out[sensor_id] = r
    return out


def cumulative_ticks(path, wheel_base=0.180, wheel_radius=0.032, ticks_per_rev=1024):
    left = 0
    right = 0
    rows = []
    imu_rows = []
    prev = path[0]
    for i, pose in enumerate(path):
        t_us = i * 20000
        if i > 0:
            x0, y0, yaw0 = prev
            x1, y1, yaw1 = pose
            ds = math.hypot(x1 - x0, y1 - y0)
            dyaw = wrap_pi(yaw1 - yaw0)
            dl = ds - dyaw * wheel_base * 0.5
            dr = ds + dyaw * wheel_base * 0.5
            left += int(round(dl / (2.0 * math.pi * wheel_radius) * ticks_per_rev))
            right += int(round(dr / (2.0 * math.pi * wheel_radius) * ticks_per_rev))
            gyro = dyaw / 0.02
        else:
            gyro = 0.0
        rows.append([t_us, left, right])
        imu_rows.append([t_us, gyro])
        prev = pose
    return rows, imu_rows


def make_path(duration_s, mode):
    n = int(duration_s * 50)
    path = []
    for i in range(n):
        t = i / 50.0
        if mode == "static_turn_scan":
            if t < 1.0:
                x, y, yaw = 0.0, 0.0, 0.0
            elif t < duration_s - 1.0:
                frac = (t - 1.0) / max(0.001, duration_s - 2.0)
                x, y, yaw = 0.04 * math.sin(frac * math.pi), 0.02 * math.sin(frac * math.pi * 0.5), math.radians(-6 + 12 * frac)
            else:
                x, y, yaw = 0.0, 0.02, 0.0
        elif mode == "straight":
            x, y, yaw = 0.04 * t, 0.0, 0.0
        else:
            x, y, yaw = 0.0, 0.0, 0.0
        path.append((x, y, yaw))
    return path


def sample_pose(path, t_us):
    idx = min(len(path) - 1, max(0, round(t_us / 20000)))
    return path[idx]


def scenario_spec(name):
    if name == "case_stable_corner":
        return {
            "segments": [(1.15, -1.20, 1.15, 0.95), (-0.95, 0.95, 1.15, 0.95), (0.45, -0.75, 1.15, -0.75)],
            "path_mode": "static_turn_scan",
            "debug_yaw_deg": 5.0,
            "confidence": 95,
            "drop": set(),
            "min_margin": 0.003,
            "min_reliability": 0.3,
            "required_consistency": 1,
        }
    if name == "case_flat_wall":
        return {
            "segments": [(1.40, -2.0, 1.40, 2.0)],
            "path_mode": "straight",
            "debug_yaw_deg": 5.0,
            "confidence": 90,
            "drop": set(),
            "min_margin": 0.02,
            "min_reliability": 0.7,
            "required_consistency": 3,
        }
    if name == "case_corridor_ambiguous":
        return {
            "segments": [(-2.0, 0.70, 2.0, 0.70), (-2.0, -0.70, 2.0, -0.70), (2.2, -0.70, 2.2, 0.70)],
            "path_mode": "straight",
            "debug_yaw_deg": 5.0,
            "confidence": 90,
            "drop": set(),
            "min_margin": 0.02,
            "min_reliability": 0.7,
            "required_consistency": 3,
        }
    if name == "case_bad_tof_confidence":
        return {
            "segments": [(1.15, -1.20, 1.15, 0.95), (-0.95, 0.95, 1.15, 0.95), (0.45, -0.75, 1.15, -0.75)],
            "path_mode": "static_turn_scan",
            "debug_yaw_deg": 5.0,
            "confidence": 12,
            "drop": set(),
            "min_margin": 0.02,
            "min_reliability": 0.7,
            "required_consistency": 3,
        }
    if name == "case_insufficient_tof":
        return {
            "segments": [(1.15, -1.20, 1.15, 0.95), (-0.95, 0.95, 1.15, 0.95), (0.45, -0.75, 1.15, -0.75)],
            "path_mode": "static_turn_scan",
            "debug_yaw_deg": 5.0,
            "confidence": 90,
            "drop": {"left", "right"},
            "min_margin": 0.02,
            "min_reliability": 0.7,
            "required_consistency": 3,
        }
    raise ValueError(name)


def generate_case(case_dir, name, duration_s):
    spec = scenario_spec(name)
    data_dir = Path(case_dir) / "csv_inputs"
    data_dir.mkdir(parents=True, exist_ok=True)
    path = make_path(duration_s, spec["path_mode"])
    enc_rows, imu_rows = cumulative_ticks(path)
    tof_rows = []
    for i in range(int(duration_s * 30)):
        t_us = i * 33333
        pose = sample_pose(path, t_us)
        ranges = tof_ranges(pose, spec["segments"])
        for sensor_id in ["front", "left", "right"]:
            if sensor_id in spec["drop"]:
                tof_rows.append([t_us, sensor_id, 4500, 6, 0])
                continue
            r = ranges[sensor_id]
            status = 0 if r <= 3.0 else 255
            raw_mm = int(round(min(r, 4.5) * 1000.0))
            confidence = spec["confidence"] if status == 0 else min(spec["confidence"], 10)
            tof_rows.append([t_us, sensor_id, raw_mm, status, confidence])
    write_rows(data_dir / "encoder.csv", ["timestamp_us", "left_ticks", "right_ticks"], enc_rows)
    write_rows(data_dir / "imu.csv", ["timestamp_us", "gyro_z_rad_s"], imu_rows)
    write_rows(data_dir / "tof.csv", ["timestamp_us", "id", "raw_range_mm", "range_status", "confidence"], tof_rows)
    return data_dir, spec


def write_config(path, data_dir, output_dir, spec):
    Path(path).write_text(f"""robot:
  wheel_base_m: 0.180
  wheel_radius_left_m: 0.032
  wheel_radius_right_m: 0.032
  encoder_ticks_per_rev: 1024

encoder:
  source: csv
  path: {data_dir / 'encoder.csv'}
  ticks_are_cumulative: true

imu:
  source: csv
  path: {data_dir / 'imu.csv'}
  mode: localization
  gyro_axis: z
  gyro_sign: 1
  gyro_bias_static_calib_s: 0
  static_gyro_max_abs_rad_s: 0.03
  yaw_fusion_alpha: 0.95

localization:
  encoder_max_wheel_speed_mps: 1.0
  gyro_lpf_alpha: 0.30
  gyro_spike_radps: 4.0
  stationary_linear_speed_mps: 0.01
  stationary_angular_speed_radps: 0.02
  gyro_bias_adapt_alpha: 0.001

tof:
  source: csv
  csv_path: {data_dir / 'tof.csv'}
  frequency_hz: 30
  min_range_m: 0.05
  max_range_m: 3.0
  confidence_min: 70
  median_window: 3
  jump_reject_m: 1.5
  mad_reject_m: 1.0
  stable_hits_required: 1

tof_devices:
  front: /dev/mt3801_front
  left: /dev/mt3801_left
  right: /dev/mt3801_right

tof_extrinsics:
  front.x_m: 0.10
  front.y_m: 0.00
  front.yaw_deg: 0
  front.fov_deg: 10
  left.x_m: 0.00
  left.y_m: 0.08
  left.yaw_deg: 90
  left.fov_deg: 10
  right.x_m: 0.00
  right.y_m: -0.08
  right.yaw_deg: -90
  right.fov_deg: 10

mapping:
  resolution_m: 0.05
  chunk_cells: 64
  initial_width_m: 10.0
  initial_height_m: 10.0
  ray_step_deg: 2.0
  hit_thickness_m: 0.08
  log_odds_occ: 0.85
  log_odds_free: -0.35
  log_odds_min: -3.5
  log_odds_max: 3.5
  occupied_thresh: 0.65
  free_thresh: 0.25
  max_cells_per_tof_update: 4096
  static_scan_stable_required: 1
  static_scan_boost: 1.2

runtime:
  localization_hz: 50
  tof_read_hz: 30
  mapping_hz: 10
  log_hz: 10

runtime_limits:
  max_linear_speed_mps: 0.25
  pause_linear_speed_mps: 0.40
  max_angular_speed_radps: 0.785
  pause_angular_speed_radps: 1.571

logging:
  output_dir: {output_dir}

map_saver:
  save_on_exit: true
  autosave_period_s: 0

tof_pose_correction:
  enabled: true
  mode: yaw_candidate
  update_hz: 5.0
  search_xy_m: 0.08
  search_xy_step_m: 0.02
  search_yaw_deg: 10.0
  search_yaw_step_deg: 1.0
  min_valid_tof: 2
  max_residual_m: 0.40
  min_margin: 0.0
  max_xy_m: 0.03
  max_yaw_deg: 2.0
  gain: 0.3
  mapping_mode_apply: false
  candidate_lpf_alpha: 0.35
  required_consistency: {spec['required_consistency']}
  consistency_yaw_deg: 2.5
  min_candidate_reliability: {spec['min_reliability']}
  min_best_second_margin: {spec['min_margin']}
  yaw_residual_curve_debug: true
  debug_pose_offset_x_m: 0.0
  debug_pose_offset_y_m: 0.0
  debug_pose_offset_yaw_deg: {spec['debug_yaw_deg']}
""")


def latest_run(output_dir):
    latest = Path(output_dir) / "latest"
    if latest.exists():
        return latest.resolve()
    runs = sorted((Path(output_dir) / "runs").glob("*"))
    return runs[-1] if runs else None


def parse_analysis(text):
    out = {}
    for line in text.splitlines():
        if ": " not in line or line.startswith("  "):
            continue
        k, v = line.split(": ", 1)
        try:
            out[k] = float(v)
        except ValueError:
            out[k] = v
    return out


def run_case(binary, work_dir, name, duration_s):
    case_dir = Path(work_dir) / name
    data_dir, spec = generate_case(case_dir, name, duration_s)
    output_dir = case_dir / "output"
    config_path = case_dir / "config.yaml"
    write_config(config_path, data_dir, output_dir, spec)
    cmd = [binary, "--config", str(config_path), "--duration-s", str(duration_s), "--output-dir", str(output_dir)]
    print("running: " + " ".join(cmd), flush=True)
    proc = subprocess.run(cmd, text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    print(proc.stdout, end="")
    print(proc.stderr, end="")
    if proc.returncode != 0:
        return proc.returncode, None, ""
    run_dir = latest_run(output_dir)
    if not run_dir:
        print(f"{name}: no run directory")
        return 1, None, ""
    analyzer = Path(__file__).with_name("analyze_candidate_yaw.py")
    analysis = subprocess.run(["python3", str(analyzer), str(run_dir)], text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    (case_dir / "analysis.txt").write_text(analysis.stdout + analysis.stderr)
    print(f"{name}: run_dir={run_dir}")
    print(analysis.stdout, end="")
    print(analysis.stderr, end="")
    if analysis.returncode != 0:
        return analysis.returncode, run_dir, analysis.stdout + analysis.stderr
    return 0, run_dir, analysis.stdout


SUMMARY_FIELDS = [
    "scenario",
    "candidate_usable_ratio",
    "multimodal_ratio",
    "flat_curve_ratio",
    "candidate_reliability_mean",
    "reject_for_insufficient_tof_count",
    "improvement_mean",
    "improvement_positive_ratio",
    "used_tof_mean",
    "expected_behavior",
    "pass",
]

EXPECTED_BEHAVIOR = {
    "case_stable_corner": "at_least_some_usable",
    "case_flat_wall": "degenerate_multimodal",
    "case_corridor_ambiguous": "degenerate_multimodal",
    "case_bad_tof_confidence": "reject_bad_tof",
    "case_insufficient_tof": "reject_insufficient_tof",
}


def metric(row, key):
    try:
        return float(row.get(key, 0.0))
    except (TypeError, ValueError):
        return 0.0


def scenario_pass(name, row):
    if name == "case_stable_corner":
        return metric(row, "candidate_usable_ratio") > 0.0 and metric(row, "candidate_reliability_mean") > 0.0
    if name == "case_flat_wall":
        return metric(row, "multimodal_ratio") >= 0.8
    if name == "case_corridor_ambiguous":
        return metric(row, "multimodal_ratio") >= 0.8
    if name == "case_bad_tof_confidence":
        return metric(row, "reject_for_insufficient_tof_count") > 0.0 and metric(row, "candidate_reliability_mean") == 0.0
    if name == "case_insufficient_tof":
        return metric(row, "reject_for_insufficient_tof_count") > 0.0 and metric(row, "candidate_reliability_mean") == 0.0
    return False


def summary_row(name, analysis):
    row = {field: "" for field in SUMMARY_FIELDS}
    row["scenario"] = name
    for key in [
        "candidate_usable_ratio",
        "multimodal_ratio",
        "flat_curve_ratio",
        "candidate_reliability_mean",
        "reject_for_insufficient_tof_count",
        "improvement_mean",
        "improvement_positive_ratio",
        "used_tof_mean",
    ]:
        row[key] = analysis.get(key, 0.0)
    row["expected_behavior"] = EXPECTED_BEHAVIOR.get(name, "unknown")
    row["pass"] = "true" if scenario_pass(name, row) else "false"
    return row


def write_summary(path, rows):
    with Path(path).open("w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=SUMMARY_FIELDS)
        writer.writeheader()
        writer.writerows(rows)

def main():
    parser = argparse.ArgumentParser(description="Run yaw_candidate CSV scenario matrix with residual-curve diagnostics.")
    parser.add_argument("--binary", required=True)
    parser.add_argument("--work-dir", default="/tmp/robot_slamd_yaw_candidate_csv_scenarios")
    parser.add_argument("--duration-s", type=float, default=5.0)
    parser.add_argument("--keep", action="store_true")
    parser.add_argument("--strict", action="store_true", help="return non-zero when any scenario pass=false")
    args = parser.parse_args()
    work_dir = Path(args.work_dir)
    if work_dir.exists() and not args.keep:
        shutil.rmtree(work_dir)
    work_dir.mkdir(parents=True, exist_ok=True)
    case_names = [
        "case_stable_corner",
        "case_flat_wall",
        "case_corridor_ambiguous",
        "case_bad_tof_confidence",
        "case_insufficient_tof",
    ]
    summary_rows = []
    runtime_failures = []
    for name in case_names:
        rc, _, analysis_text = run_case(args.binary, work_dir, name, args.duration_s)
        if rc != 0:
            runtime_failures.append(name)
            analysis = {}
        else:
            analysis = parse_analysis(analysis_text)
        summary_rows.append(summary_row(name, analysis))
    summary_path = work_dir / "scenario_summary.csv"
    write_summary(summary_path, summary_rows)
    print(f"scenario_summary_csv: {summary_path}")
    failed = [row["scenario"] for row in summary_rows if row["pass"] != "true"]
    if runtime_failures:
        print("runtime_failed_scenarios: " + ",".join(runtime_failures))
    if failed:
        print("failed_scenarios: " + ",".join(failed))
    else:
        print("failed_scenarios: none")
    if runtime_failures:
        return 1
    if args.strict and failed:
        return 1
    return 0


if __name__ == "__main__":
    raise SystemExit(main())

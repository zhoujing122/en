...
传感器误差预算和标定验收范围见 `SENSOR_ERROR_BUDGET.md`。本文档中的 ToF 要求必须与该误差预算一起执行。
...
```sh
/userdata/robot_slamd/robot_slamd \
  --config /userdata/robot_slamd/config.board_sim_spin_scan_smoke.yaml \
  --duration-s 20 \
...
config.board_sim_spin_scan_smoke.yaml
config.hw_v1_5_smoke.yaml
config.hw_yaw_candidate.yaml
BOARD_DEPLOY_SERIAL.md
TOF_REQUIREMENTS.md
一次板端 smoke run_dir
map.pgm
map.yaml
